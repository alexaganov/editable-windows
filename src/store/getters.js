const getters = {
  ...
  listOfWindows: (state, getters) => getters.windows.map(window => {
    return {
      name: window.name,
      id: window.id,
  ...
  lastAddedWindow: (state, getters) => 
    state.windows.find(window => window.id === getters.lastCreatedWindowId) || {},
  ...
  numOfRemovedWindows: (state) => 
    state.windows.filter(window => window.isRemoved).length,
}
